...
class blog{
	...
	function detail($cat_only=0,$cat_name=''){
		global $matched_advice;
		global $found_advice;
		
		switch($cat_only){
		case 0:
		default:
		if($matched_advice && isset($found_advice[1]) && $found_advice[1]):			
			$return=$this->db->detail($this->table,'',"alias = '". mysql_real_escape_string($found_advice[1])."'");
			return $return;		
		else:
		...
		if($cat_name):			
			...
			$return=$this->db->detail('8x_blog_category','',"name = '". mysql_real_escape_string($cat_name)."'");
			return $return;		
		else:
			return $this->db->detail('8x_blog_category','',$this->key.' = '. intval($_GET['id']));
		endif;
		break;
		}
    ...
    function select($blog_id = '',$script = ''){
			...
			$temp .= '<option value="0">Select Advice Center Category</option>';
			foreach($row as $row){
