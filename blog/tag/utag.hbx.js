//v.1
//hbx.js,HBX2.5,COPYRIGHT 1997-2007 VISUALSCIENCES,INC. ALL RIGHTS RESERVED. U.S.PATENT No.6,393,479B1 & 6,766,370. INFO:http://visualsciences.com/privacy
/* INSERT CUSTOM EVENT CODE HERE */
// custom code for 2.5 release to send linkviews on page view until the backend is ready
...
function __A(a){return unescape(a)}
...
_VC("pn|PUT+PAGE+NAME+HERE,mlc|CONTENT+CATEGORY,elf|n,dlf|n,dft|n,pndef|title,ctdef|full,cp|null,hcn|")}
...
_XT("PrePV",_ar);
if(_D("onlyMedia")!="y")_hbi.src=_ar+"&hid="+Math.random();
_hbq=_IS(_ar,0,_II(_ar,"&hec"));
_XT("PostPV",_ar);
_hbE=_NA()
}
...
//tealium universal tag - utag.sender.hbx ut2.2.20101019, Copyright 2010 Tealium.com Inc. All Rights Reserved.

var hbx={_N:"pv",_C:"0",acct:utag.cfg.hbx_acct,gn:utag.cfg.hbx_gn,vpc:"HBX0100u",lt:"none"};_hbE=[hbx];_hbEC=1;

try{

utag.sender["hbx"].INIT=function(a,b){
    var __u=b;
    if(typeof _pn!="undefined")_hbSet("n",__u.page);
    else hbx.pn=__u.page;
    
    if(typeof _mlc!="undefined")_hbSet("vcon",__u.section);    
    else hbx.mlc=__u.section;
    
    try{
      if(typeof __u.search_term!="undefined"){
        _hbE[_hbEC]={
          _N:"search",
          _C:"0",
          keywords:__u.search_term,
          results:__u.search_results,
          attr1:__u.search_location,
          attr2:__u.search_type,
          attr3:__u.heading,
          attr4:__u.directory
        }
        _hbEC++;
    ...
    try{
      _hbE[_hbEC]={
        ...
        _C:"0",
        c5: __u.mlr,
        c6: __u.survey_code+"|"+__u.what_where,
        c7: __u.survey_code+"|"+__u.survey_sat,
        c8: __u.survey_code+"|"+__u.survey_reason,
        c9: __u.search_term+"|"+__u.relatedsearch_flag,
        c10:__u.listing_id+"|"+__u.video_pct,
        c11:__u.listing_id+"|"+__u.photo,
        c12:__u.related_search,
        c13:__u.cobrand+"|"+__u.language,
        c14:__u.survey_id,
        c15:__u.search_category,
        c16:__u.search_term+"|"+__u.search_refine,
        c17:__u.what_where+"|"+__u.position_address,
        c18:__u.what_where+"|"+__u.position_number,
        c19:__u.search_detail+"|"+__u.search_category,
        c20:__u.listing_id+"|"+__u.video_error,
        c21:__u.video_menu,
        c22:__u.what_where+"|"+__u.disambiguation,
        c23:__u.eco_disp+"|"+__u.eco_imp,
        c24:__u.eco_link+"|"+__u.search_term,
        c25:__u["cp.formabandon"]
      }
      var j=_hbE[_hbEC],k=[];
      for(var i in j){
        if(typeof j[i]!="function")k.push(i);
      }
      
      for(i=0;i<k.length;i++){
        if(typeof j[k[i]]=="undefined")
            delete _hbE[_hbEC][k[i]];
            
        else{ 
            j[k[i]]=j[k[i]].replace(/undefined/g,"");
            if(j[k[i]]=="|"||j[k[i]]=="")
                delete _hbE[_hbEC][k[i]];
            else if(j[k[i]].indexOf("|")==j[k[i]].length-1||j[k[i]].indexOf("|")==0)
                _hbE[_hbEC][k[i]]=j[k[i]].replace("|","");
        }
      }
      _hbEC++;
    }catch(e){}
    ...
    _rf=_A(eval("document"+__r));_et=0;_oe=0;_we=0;_ar="";_hM=(!(_II(navigator.userAgent,"Mac")>-1));_tls="";_bv=parseInt(navigator.appVersion);_bv=(_bv>99)?(_bv/100):_bv;
    __f="";_hrat=_D("hra");_hra="";__c="catch(_e)";_fa=0;_hlfs=0;_hoc=0;_hlf='';_ce='';_ln='';_pl='';_bn=navigator.appName;_bn=(_II(_bn,"Microsoft")?_bn:"MSIE");_bnN=(_bn=="Netscape");
    _bnI=(_bn=="MSIE");_hck="*; path=/; "+(_D("cpd")&&_D("cpd")!=""?(" domain=."+_D("cpd")+"; "):"")+_ex;_N6=(_bnN&&_bv>4);_I5=false;_ss="na";_sc="na";_sv=11;_cy="u";_hp="u";_nc="";_np="";_tp=_D("ptc");
    ...
    _mlc=_PP(_mlc,0,_II( _mlc,";"),"CONTENT+CATEGORY");
    _pn=_PP(_pn,1,_II(_pn,";"),"PUT+PAGE+NAME+HERE");

    _PV();
    
    //POST Handling
    if(_ar.length>2048&&navigator.userAgent.indexOf('MSIE')>=0){
        a=_ar.split("?");
        a=a[1].split("&");
        b={};
        ...
        utag.sender["hbx"].sendpost("view",b);
    }
...
utag.sender["hbx"].cs=(function(u){
    u=utag.sender["hbx"];
    
    //define tracked events
    u.ev={view:1,link:1,download:1,exitlink:1}
    ...
    u.map={
        ...
        video_pct:"cv.c10",
        photo:"cv.c11",
        position_address:"cv.c17",
        position_number:"cv.c18",
        video_error:"cv.c20",
        eco_disp:"cv.c23",
        eco_link:"cv.c24",
        disambiguation:"cv.c22", //fixed by van
        "cp.formabandon":"cv.c25"
    }
    ...
    u.extend.push(
        function(o,a,b,c){
            if(a=="link"){
                if(b.photo)b.photo = b.listing_id+"|"+b.photo;
                if(b.video_pct)b.video_pct = b.listing_id+"|"+b.video_pct;
                if(b.video_error)b.video_error = b.listing_id+"|"+b.video_error;
                if(b.position_number)b.position_number=utag.last.what_where+"|"+b.position_address;
                if(b.position_address)b.position_address=utag.last.what_where+"|"+b.position_address;
                if(b.eco_link)b.eco_link+="|"+utag.last.search_term;
                if(b.disambiguation)b.disambiguation=utag.last.what_where+"|"+b.disambiguation; //fixed by van
            }
        }
    );
    ...
    u.send=function(a,b,c,d,e,f){
        ...
        if((u.ev[a]||typeof u.ev["all"]!="undefined")&&u.ev[a]!=0){
            ...
            if(a=="link"){
                ...
                b.link_attr1=(b.link_attr1)?b.link_attr1:"";
                _hbLink(b.link_name,b.link_attr1)
            }
            else if(a=="download")_hbDownload(b.lid);
            else if(a=="exitlink")_hbExitLink(b.lid);
        }
    }        
})();

utag.sender["hbx"].sendpost=function(a,b,c,d,e,f,i,t){
    d=document;
    t=d.getElementsByTagName("body")[0];

    //create iframe target
    i=document.createElement("div");
    i.innerHTML="<iframe id='utag_hbx_iframe' name='utag_hbx_iframe' src='about:blank' style='display:none'></iframe>";
    t.appendChild(i);

    //create form object
    f=document.createElement("FORM");
    f.name="utag_hbx_form";
    f.action="//"+hbx.gn+"/HGct?_cb="+Math.random();
    f.method="post";
    f.target="utag_hbx_iframe";
    ...
    t.appendChild(f);
    f.submit();
    setTimeout('document.getElementById("utag_hbx_iframe").src="about:blank"',2000);
}

utag.loader.LOAD("hbx")
}catch(e){};
