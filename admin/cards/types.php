...
if ($_GET['id']){	
	$row2="";$oldid=$_GET['id'];
	$row = $cards_type->detail();
	if((int)$_GET['id']!=''){$_GET['id']=4;$row2=$cards_type->detail();$_GET['id']=$oldid;$keysx=explode(",",$row2['cards_list']);if(is_array($keysx)){ if($row['tbl_id']!=''){$topnames=explode("@||@|@",$row['top_type_name']);}$tx=0;foreach($keysx as $kk=>$vv){$ftdlist[$vv]=$topnames[$tx];$tx++;}}}
	$row = $cards_type->detail();

	if($row['top_type_name'] && $row['top_list']){
...
if($row['ordering']!=''){
	...
	echo $temp->title('Add New Card Type:');
    ...
    if($row['tbl_id']==4){
		echo $temp->input_text('Top Type Name','top_type_name2',$row['top_type_name2']);
    }
    ...
    ?>
    <tr>
        <td colspan="2" style="padding-left: 490px;color:  #0B9DC8; font-weight:  bold; font-size: 10px;">Card Order <?if($row['tbl_id']!=4){?><span style="padding-left: 30px;"> Featured</span><?}?></td>
            ...
            if($row['tbl_id']!=4 || (int)$_GET['id']==4){
              ...
              $holdcards[$holdc].="<span style=\"margin-left: 45px;\">
               ...
               $holdcards[$holdc].="</div>
                ...
                echo "<b>Normal Cards</b><br>".$holdcards['reg'];
                }
...
<script language="javascript">
	function checkinput(){
		<? 	echo $check->check_blank('name','Type name cannot be blank.');
