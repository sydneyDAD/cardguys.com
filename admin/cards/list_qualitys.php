...
if ($_POST['option'] == 'delete'){
	...
	$cards_quality->delete();
}
$db->update_status($cards_quality->table,$cards_quality->key);
$option = 'delete';$i = 1;
$row = $cards_quality->lists();

?>
<a style="color: #CA4102; font-weight: bold; font-size: 14px; text-decoration: underline;" href="<?=$sitelink?>admin/index.php?module=cards&view=qualitys">Add New Quality</a>
<?
	...
	foreach ($row as $row){
		...
		echo $temp->td($template->show_image($row->image,$cards_bank->pathadm.'resize/','width=60px'),'60px','center');
		echo $temp->td('<a href="index.php?module='.$cards_bank->module.'&view=qualitys&id='.$row->tbl_id.'"><b>'.$row->name.'</b></a>');
