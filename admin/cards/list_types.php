...
$db->update_status($cards_type->table,$cards_type->key);
$option = 'delete';$i = 1;
$row = $cards_type->lists();

?>
<a style="color: #CA4102; font-weight: bold; font-size: 14px; text-decoration: underline;" href="<?=$sitelink?>admin/index.php?module=cards&view=types">Add New Type</a>
<?
...
if($_SERVER['REMOTE_ADDR']=='loclal_ip_address' || $_SERVER['REMOTE_ADDR']=='development_ip_address'){ //for testing only
	...
	foreach ($row as $row){
		...
		echo $temp->td($template->show_image($row->image,$cards_type->pathadm.'resize/','width=60px'),'60px','center');
		echo $temp->td('<a href="index.php?module='.$cards_type->module.'&view=types&id='.$row->tbl_id.'"><b>'.$row->name.'</b></a>');
